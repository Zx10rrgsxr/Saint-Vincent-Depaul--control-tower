...
with tab3:
    ...
    area_filter = a1.multiselect('Preferred Area', sorted(model_df['Preferred_Area'].unique().tolist()), default=sorted(model_df['Preferred_Area'].unique().tolist()))
    bucket_filter = a2.multiselect('Capacity Bucket', ['Premium', 'Standard', 'Reserve'], default=['Premium', 'Standard', 'Reserve'])
    item_filter = a3.multiselect('Item Type', sorted(model_df['Item_Type'].astype(str).unique().tolist()), default=sorted(model_df['Item_Type'].astype(str).unique().tolist()))

    view = model_df[
        model_df['Preferred_Area'].isin(area_filter) &
        model_df['Capacity_Bucket'].isin(bucket_filter) &
